...
import { BookOpen, Users, Package, PenLine } from 'lucide-react';

const BACKEND = import.meta.env.VITE_API_URL || 'http://localhost:5000';

const StatCard = ({ Icon, label, value, sub, color, iconColor, textColor }) => (
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        <StatCard Icon={BookOpen} label="Libra"   value={stats.books}   sub={`${stats.totalStock} copë në stok`} color="bg-blue-50"   iconColor="text-blue-600"   textColor="text-blue-700" />
        <StatCard Icon={Users}   label="Klientë" value={stats.clients} sub="të regjistruar"                    color="bg-green-50"  iconColor="text-green-600"  textColor="text-green-700" />
        <StatCard Icon={Package} label="Porosi"  value={stats.orders}  sub={`${stats.statusCounts?.Pending ?? 0} pending`}          color="bg-purple-50" iconColor="text-purple-600" textColor="text-purple-700" />
        <StatCard Icon={PenLine} label="Autorë"  value={stats.authors} sub={`${stats.categories} kategori`}   color="bg-orange-50" iconColor="text-orange-600" textColor="text-orange-700" />
      </div>
                ...
                {stats.lowStock.map(b => (
                  <div key={b.liber_id} className="flex items-center gap-3">
                    {b.foto_url
                      ...
                      : <div className="w-8 h-11 bg-gray-100 rounded flex items-center justify-center shrink-0"><BookOpen size={16} className="text-gray-400" strokeWidth={1.5} /></div>}
