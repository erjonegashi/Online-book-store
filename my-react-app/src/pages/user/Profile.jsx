...
export default function Profile() {
  ...
  const handlePasswordChange = async e => {
    ...
    setPwSaving(true);
    try {
      await userApi.put('/user/me', { ...profile, password: pwForm.next });
  ...
  return (
    <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-10">
      ...
      <div className="flex gap-1 bg-gray-100 rounded-xl p-1 mb-6 w-fit">
        {[['orders', '📦 My Orders'], ['settings', '⚙️ Settings']].map(([key, label]) => (
                ...
                <button
                  ...
                  <div className="flex items-center gap-5 flex-wrap min-w-0">
                    ...
                    <span className={`inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full text-xs font-semibold border shrink-0 ${STATUS_STYLES[order.statusi] || 'bg-gray-50 text-gray-600 border-gray-200'}`}>
                      {STATUS_ICONS[order.statusi]} {order.statusi}
                    </span>
                  </div>
                  <svg className={`w-4 h-4 text-gray-400 shrink-0 transition-transform duration-200 ${expanded === order.porosi_id ? 'rotate-180' : ''}`}
