...
exports.getAll = async (_req, res) => {
  ...
  catch (err) { console.error('[PromocionCtrl] getAll:', err.message); res.status(500).json({ error: 'Kërkesa dështoi. Provo përsëri.' }); }
};
...
exports.getById = async (req, res) => {
  try {
    ...
    res.json(promocioni);
  } catch (err) { console.error('[PromocionCtrl] getById:', err.message); res.status(500).json({ error: 'Kërkesa dështoi. Provo përsëri.' }); }
};
...
exports.create = async (req, res) => {
  ...
  try {
    ...
    console.error('[PromocionCtrl] create:', err.message); res.status(500).json({ error: 'Kërkesa dështoi. Provo përsëri.' });
  }
};
...
exports.update = async (req, res) => {
  ...
  } catch (err) { console.error('[PromocionCtrl] update:', err.message); res.status(500).json({ error: 'Kërkesa dështoi. Provo përsëri.' }); }
};
...
exports.remove = async (req, res) => {
  ...
  } catch (err) { console.error('[PromocionCtrl] remove:', err.message); res.status(500).json({ error: 'Kërkesa dështoi. Provo përsëri.' }); }
};
